...
def create_matrix_lemma(corpus):
    ...
    with open("word_frequencies_lemma.txt", "w", encoding="utf-8") as file:
        for term, count in sorted_word_counts:
            file.write(f"{term}: {count}\n")

    texts = [[token for token in text if word_count_dict[token] > 19] for text in corpus]
    bm25 = BM25()
    bm25.fit(texts)
...
corpus = read_document("A_lemma.txt")
corpus.extend(read_document("B_lemma.txt"))
corpus.extend(read_document("C_lemma.txt"))
matrix, terms=create_matrix_lemma(corpus)
...
print(lemmot.shape)
print("before A")
lemmot.iloc[:5000].to_excel('A_lemma.xlsx', index=True,encoding='utf-8')
print("A_lemma excel created")
lemmot.iloc[5000:10000].to_excel('B_lemma.xlsx', index=True,encoding='utf-8')
lemmot.iloc[10000:].to_excel('C_lemma.xlsx', index=True,encoding='utf-8')
...
with open("IG_scores_ALemma.txt", "w", encoding="utf-8") as file:
    for term, score in ig_scores_A.items():
        file.write(f"{term}: {score}\n")

ig_scores_B=calculate_IG(B_matrix,Y_lemmots)
with open("IG_scores_BLemma.txt", "w", encoding="utf-8") as file:
    for term, score in ig_scores_B.items():
        file.write(f"{term}: {score}\n")
ig_scores_C=calculate_IG(C_matrix,Y_lemmots)
with open("IG_scores_CLemma.txt", "w", encoding="utf-8") as file:
...
mig_scores=calculate_MIG(matrix,Y)
with open("MIG_scores.txt", "w", encoding="utf-8") as file:
    for feature_index, score in mig_scores.items():
        file.write(f"{feature_index}: {score}\n")

mig_scores_A=calculate_MIG(A_matrix,Y_lemmots)
with open("MIG_scores_ALemma.txt", "w", encoding="utf-8") as file:
    for term, score in mig_scores_A.items():
        file.write(f"{term}: {score}\n")

mig_scores_B=calculate_MIG(B_matrix,Y_lemmots)
with open("MIG_scores_BLemma.txt", "w", encoding="utf-8") as file:
    for term, score in mig_scores_B.items():
        file.write(f"{term}: {score}\n")
mig_scores_C=calculate_MIG(C_matrix,Y_lemmots)
with open("MIG_scores_CLemma.txt", "w", encoding="utf-8") as file:
...
df.to_excel("IG_MIG_ALemma.xlsx", index=False)
...
df_B.to_excel("IG_MIG_BLemma.xlsx", index=False)
...
df_C.to_excel("IG_MIG_CLemma.xlsx", index=False)
...
# Write DataFrame to xlsx file
df.to_excel("IG_MIG_SHAP_ALemma.xlsx", index=False)
...
# Write DataFrame to xlsx file for B set
df_B.to_excel("IG_MIG_SHAP_BLemma.xlsx", index=False)
...
# Write DataFrame to xlsx file for C set
df_C.to_excel("IG_MIG_SHAP_CLemma.xlsx", index=False)
